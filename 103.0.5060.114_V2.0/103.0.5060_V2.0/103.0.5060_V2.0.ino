...
#define device_version 103 // Change to the ChromeOS version you expect to use with Centipede; Changes have been reported in the following ranges 58-68, 69, 70
...
#define phaseTwoAuthentication 0 //[0,1,2,3,4,5,6] Set to 0 for automatic, 1 for EAP-MD5, 2 for MSCHAP(v2 pre v69; v1 V69+, 3 for MSCHAPv2, 4 for PAP, 5 for CHAP, 6 for GTC; v69+)
...
#define VERSION_103 (device_version >= 103)
...
void loop() { // Main Function - workflow is called within loop();
  ...
  if (digitalRead(buttonPin) == 1 ) { // Check for debugging. If not debugging, run the program
    ...
    if (setWiFi){
      ...
      enterEnrollment(); //Moved into main loop due to different order of screens on version 87
    }
...
void welcomeScreen() {
  repeatKey(KEY_TAB, 2 - selected_on_boot);
  wait(2);
